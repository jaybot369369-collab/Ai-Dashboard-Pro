...
const DojoTab = (() => {
  ...
  /* ── Format helpers ─────────────────────────────────── */
  const dp   = sym => sym && sym.includes('BTC') ? 2 : sym && sym.includes('XRP') ? 4 : 4;
  ...
  function renderPairChips() {
    const all = [...PROTECTED, ..._custom.filter(s => !PROTECTED.includes(s))];
...
Be concise but specific — every "key_level" must be a price (e.g. "63420" or "63,400 swing high"). Every "rationale" must reference what you see in the data, not generic ICT theory.`;
  ...
  function renderCustomTickers() {
    ...
    const chips = _custom.map(s => `<span class="dojo-pair-chip" style="cursor:default">
      <code>${esc(s)}</code>
